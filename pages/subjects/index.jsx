        ...
        <>
            ...
            { auth.user && (
                <>
                        ...
                        <div className="flex flex-row flex-wrap gap-8">
                            {
                                subjects.length > 0 && subjects.map((subject) => {
                                    return (
                                        <SubjectCard key={subject.id} subject={subject} mutate={mutate} updateHandler={handleUpdateModal}/>
                                    )
                                })
                            }
                            <div onClick={() => setShowAddModal(true)} className="p-6 w-72 h-32 max-w-sm border border-dashed border-8 rounded shadow-md border-nord9 cursor-pointer">
                                <div className="h-full flex justify-center items-center">
                                    <HiPlus className="mr-2 h-12 w-12 fill-nord9" />
                                    <h5 className="flex flex-row mb-2 text-xl md:text-2xl font-bold tracking-tight text-nord9 break-all">Add Subject</h5>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    {
                        showUpdateModal && (
                            <UpdateSubjectModal subjectData={updateSubjectData} modalHandler={handleUpdateModal} mutate={mutate} />
                        )
                    }
                </>
            )
