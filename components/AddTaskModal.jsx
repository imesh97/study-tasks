...
const currentDate = () => { // get current date
            ...
            <div className="relative w-auto my-6 mx-auto max-w-3xl">
              <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-nord1 text-nord6 outline-none focus:outline-none">
                <div className="relative p-6">
                    <div className="">
                        <p className="text-lg font-semibold leading-relaxed">New Task:</p>
                        <div className="my-2">
                            <input value={task} onChange={(e) => setTask(e.target.value)} type="text" placeholder="Enter task title..." className="px-3 py-3 placeholder-nord4 text-nord1 relative bg-nord6 rounded-lg text-sm border border-nord2 outline-none focus:outline-none focus:ring w-full"/>
                        </div>
                    </div>
                    <div className="flex">
                        <div className="">
                ...
                <div className="flex items-center justify-end p-6 border-t border-solid border-nord2 rounded-b">
                  <button className="text-nord11 background-transparent font-bold uppercase px-6 py-2 text-sm outline-none focus:outline-none mr-1 mb-1 ease-linear transition-all duration-150" type="button" onClick={() => modalHandler(false)}>Close</button>
                  <button type="button" onClick={() => handleAdd(newTask)} className={"bg-nord8 text-nord1 active:bg-nord8 font-bold uppercase text-sm px-6 py-3 rounded shadow hover:shadow-lg outline-none focus:outline-none mr-1 mb-1 ease-linear transition-all duration-150 " + (!isFilled() ? "cursor-not-allowed" : "")} disabled={!isFilled()}>Add task</button>
                </div>
              </div>
            </div>
          </div>
