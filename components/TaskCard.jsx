...
export default function TaskCard({ task, mutate, updateHandler }) {
  ...
  const taskDate = () => {
    ...
    if (daysDiff < 0) return "expired (" + dateString + ")";
      ...
      return "in " + daysDiff + " days (" + dateString + ")";
    return dateString;
  };
  ...
  return (
    <>
      <div className="w-72 max-w-sm p-6 border border-gray-200 rounded-lg shadow-md bg-indigo-900 border-none">
        ...
        <p className="flex items-center mb-2 font-normal text-indigo-100 break-all">
          <HiOutlineClock className="h-6 w-6 mr-2" />
          {taskDate()}
        </p>
        <p className="flex items-center mb-2 font-normal text-indigo-100 break-all">
          <HiOutlineBookOpen className="h-6 w-6 mr-2" />
          {task.subject.name}
        </p>
        <div className="flex justify-end">
          <label className="inline-flex relative items-center mr-2 cursor-pointer">
            ...
            />
            <div className="w-11 h-6 bg-indigo-500 rounded-full peer peer-focus:ring-green-300  peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-0.5 after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-indigo-9000"></div>
          </label>
          <button
